...
Explanation Toolkit could always use more documentation, whether as part of the
official Explanation Toolkit docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `Explanation Toolkit` for local development.

1. Fork the `Explanation Toolkit` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/explanation-toolkit.git

3. Install your local copy into a virtualenv. Assuming you have virtualenvwrapper installed,
   this is how you set up your fork for local development::

    $ mkvirtualenv explanation-toolkit
...
2. The tests that cover a module called ``explanation_toolkit/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/explanation_toolkit/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_explanation_toolkit
...
2. Update the version in ``setup.cfg``, ``explanation_toolkit/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``explanation_toolkit/__init__.py``
...
.. _Travis Build Status page: https://travis-ci.org/DAI-Lab/explanation-toolkit/pull_requests
