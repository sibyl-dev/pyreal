...
class LocalSAXTransformer(Transformer):
    """
    ...
    def paa_transform(self, x, n_timestamps, std_thresh=1e-3, save_stats=True):
        ...
        # break_indices_matrix has shape (n_sequences, word_length+1)
        # it calculates the break points for each generated word
        ...
        # the subtraction between `break_indices` is to calculate the length of each character
        paa = (
            paa_sum / (break_indices[None, None, None, 1:] - break_indices[None, None, None, :-1])
            - seqMean[:, :, :, None]
...
class GlobalSAXTransformer(Transformer):
    """
    Symbolic Aggregate approXimation. Transform an entire signal into a single long SAX word.

    Args:
        n_bins (Integer, default = 4):
            The "discretized-ness" of the output word.

        steps_per_char (Integer):
            The number of steps represented by each character in the resulting signal.
    ...
    def __init__(self, n_bins=4, steps_per_char=5, **kwargs):
        self.n_bins = n_bins
    ...
    def paa_transform(self, x, n_timestamps, std_thresh=1e-3, save_stats=True):
        ...
        Args:
            ...
            std_thresh (float):
                Small number to stop values from blowing up after normalization.
        Returns:
            ndarray of shape (n_instances, n_variables, word_length)
                PAA representation of series.
        """
        if len(x.shape) != 3:
            raise ValueError(
        ...
        # calculate breakpoints for PAA segments
        step = n_timestamps // self.steps_per_char
        res = n_timestamps % self.steps_per_char
        break_indices = [*range(0, n_timestamps, step)]
        # handle divisibility
        if res == 0:
            break_indices.append(n_timestamps)
            break_indices = np.array(break_indices)
        else:
            break_indices = np.array(break_indices)
            break_indices[-res:] += 1 + np.arange(res)

        # seqMean and seqStd have shape (n_instances, n_variables)
        seqMean = np.mean(x, axis=-1)
        seqStd = np.std(x, axis=-1)

        # paa_sum and paa has shape (n_instances, n_variables, word_length)
        paa_sum = self.cum_sum[..., break_indices[1:]] - self.cum_sum[..., break_indices[:-1]]

        paa = (
            paa_sum / (break_indices[None, None, 1:] - break_indices[None, None, :-1])
            - seqMean[:, :, None]
        ) / (seqStd[:, :, :, None] + std_thresh)
