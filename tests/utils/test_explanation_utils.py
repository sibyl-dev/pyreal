...
def test_get_top_contributors_contributions():
    ...
    absolute = explanation_utils.get_top_contributors(exp, num_features=2, select_by="absolute")
    expected_absolute = exp.iloc[[4, 0], :]
    assert_frame_equal(absolute, expected_absolute)

    max = explanation_utils.get_top_contributors(exp, num_features=2, select_by="max")
    expected_max = exp.iloc[[0, 1], :]
    assert_frame_equal(max, expected_max)

    min = explanation_utils.get_top_contributors(exp, num_features=2, select_by="min")
    expected_min = exp.iloc[[4, 3], :]
    assert_frame_equal(min, expected_min)

    min_3 = explanation_utils.get_top_contributors(exp, num_features=3, select_by="min")
