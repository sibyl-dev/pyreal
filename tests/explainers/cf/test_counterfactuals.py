...
def test_produce():
    x = pd.DataFrame([[1, 1, 1], [10, 10, 10], [-10, -10, -10]])
    ...
    result = explainer.produce(
        pd.DataFrame([[0.0, 0.0, 0.0]]), num_examples=2, target_prediction=10
    )
    print(result.get_examples())
    # result_series = explainer.produce(pd.Series([0, 1, 0]), n=2, target_prediction=3)
    expected = pd.DataFrame([[1, 2, 0]])
    assert_frame_equal(expected, result.get_examples().iloc[0], check_dtype=False)
