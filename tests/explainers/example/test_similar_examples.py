...
def test_produce(dummy_model, fast):
...
@pytest.mark.parametrize("fast", [False, True])
def test_produce_with_transforms(regression_one_hot_with_interpret, fast):
...
@pytest.mark.parametrize("fast", [False, True])
def test_produce_multiple_with_transforms(regression_one_hot_with_interpret, fast):
    x = pd.DataFrame([[2, 1, 3], [4, 3, 4], [6, 7, 10]], columns=["A", "B", "C"])
    y = pd.Series([1, 2, 3])
    explainer = SimilarExamples(
        model=regression_one_hot_with_interpret["model"],
...
@pytest.mark.parametrize("fast", [False, True])
def test_produce_with_standardize(dummy_model, fast):
