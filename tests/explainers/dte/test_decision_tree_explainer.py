...
def test_produce_decision_tree_classification_no_transforms(
    classification_no_transform_tree,
):
...
def helper_produce_decision_tree_classification_no_transforms(explainer, model):
    ...
    assert (
        tree_object.predict(model["x"].to_numpy()) == model["y"].to_numpy().ravel()
    ).all()
...
def test_produce_decision_tree_regression_no_transforms_with_size(
    regression_no_transforms,
):
    model = regression_no_transforms
    dte = DecisionTreeExplainer(
        model=model["model"],
...
def test_produce_decision_tree_classification_with_size(
    classification_no_transform_tree,
):
    model = classification_no_transform_tree
    dte = DecisionTreeExplainer(
...
def helper_produce_decision_tree_classification_no_transforms_with_size(
    explainer, model
):
    tree_object = explainer.produce()
    assert tree_object.feature_importances_.shape == (
