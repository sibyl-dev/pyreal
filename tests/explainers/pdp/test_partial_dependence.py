...
# def test_produce_partial_dependence_classification_no_transforms(
#     binary_classification_no_transforms,
# ):
#     model = binary_classification_no_transforms
#     features = model["x"].columns[:-1]
#     pdpe = PartialDependenceExplainer(
#         model=model["model"],
#         x_train_orig=model["x"],
...
#     helper_partial_dependence_classification_no_transforms(pdpe, model)
#     helper_partial_dependence_classification_no_transforms(pdp, model)


# def helper_partial_dependence_classification_no_transforms(explainer, model):
#     pdp_object = explainer.produce().get()
#     if isinstance(pdp_object.feature_names, pd.Index):
#         assert pdp_object.feature_names.tolist() == model["x"].columns[:-1].tolist()
#     else:
