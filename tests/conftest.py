...
def transformer_test_data():
    x = pd.DataFrame(
        [[2, 1, 3, 9], [4, 3, 4, 0], [6, 7, 2, 2]], columns=["A", "B", "C", "D"]
    )
    columns = ["C", "A"]
...
@pytest.fixture()
def all_models(
    regression_no_transforms, regression_one_hot, classification_no_transforms
):
...
def regression_no_transforms(test_root):
    ...
    model_no_transforms_filename = os.path.join(
        test_root, "data", "model_no_transforms.pkl"
    )
